...
<head>
	...
	?>
	<title>Format Konsep Tema</title>
	<style type="text/css">
		body{
	...
	<center>
		...
		<table width="625">
			<tr>
				<td>
					<br>
					<p align="center"><b><font face="Arial" size="2.5"> KONSEP TEMA PENCEGAHAN MALADMINISTRASI </font></b></p> 
				</td>
			</tr>
		...
		<?php
			$id=$_GET['id'];
			$query = mysqli_query($db,"SELECT * FROM formkonseptema join substansi on formkonseptema.id_substansi=substansi.id_substansi WHERE id_konseptema='$id'")or die(mysqli_error($db));
			while($data = mysqli_fetch_array($query)){ 
			
		?>
			<tr>
		       <td>
			       <!--<font size="2">Substansi:<br>Tema/Judul:</font> -->
			       
				   <table>
					<tbody>
					<tr><td  width="80"><font size=2>Substansi</font></td><td width="10">:</td><td><font size=2><?php echo $data['jenis_substansi'] ?></font></td></tr>
					<tr><td><font size=2>Tema/Judul</font></td><td>:</td><td><font size=2><?php echo $data['tema'] ?></font></td></tr>
					</tbody></table>
		...
		<?php
			$id=$_GET['id'];
			$query = mysqli_query($db,"SELECT * FROM formkonseptema join user on formkonseptema.iduser=user.iduser WHERE id_konseptema='$id'")or die(mysqli_error($db));
			while($data = mysqli_fetch_array($query)){ 
			
		?>
			<tr>
			<td>
			       <font size=2> 1. Abstraksi</font></td>
				   </tr>
			</table>
			<table class="table-bordered">
			<tr><td width="550" ><font size=2><?php echo $data['abstraksi'] ?></font></td> </tr>
				   </table>
		<br>
		<table width="590" >
			<tr>
		       <td>
			   <font size=2> 2. Alur</font></td>
		       </td>
		    </tr>
		</table>
		<table class="table-bordered" > 
		<tr> <td width="550" ><font size=2><?php echo $data['alur'] ?></font></td> </tr>
				   </table>
		<table width="600">
			<?php
